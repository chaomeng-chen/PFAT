...
class WeightsAccountant:
    def __init__(self,sess,model,Sigma, real_round, N):

        self.Weights = [np.expand_dims(sess.run(v), -1) for v in tf.trainable_variables()]
        # self.Weights = [sess.run(v) for v in tf.trainable_variables()]
    ...
    def compute_updates(self):
        ...
        # self.Updates = [self.Weights[i] - self.global_model[i] for i in range(self.num_weights)]
        self.Weights = None
    ...
    def compute_norms(self):
        ...
        # self.Norms = [np.sqrt(np.sum(np.square(self.Updates[i]))) for i in range(self.num_weights)]
    ...
    def clip_updates(self):
        ...
        if self.C != 0:
            for i in range(self.num_weights):
                self.median[i][:] = self.C
        # self.median = [min(abs(self.Norms[i]/10/np.min(self.Updates[i])), abs(self.Norms[i]/10/np.max(self.Updates[i]))) for i in range(self.num_weights)]
        # self.median = [self.Norms[i]/10/np.max(abs(self.Updates[i])) for i in range(self.num_weights)]
        # end original
        ...
        # factor = [1 for i in range(self.num_weights)]
        for i in range(self.num_weights):
    ...
    def Update_via_GaussianMechanism(self, sess, Acc, FLAGS, Computed_deltas):
        # original/1.0 每轮训练选择一个模型，数据集非IID，噪声算法是自带算法
        # self.clip_updates()
        ...
        # 2.0 每台机器选择一个模型，数据集非IID，无噪声计算
        # self.clip_updates()
        # New_weights = [self.global_model[i]+self.Updates[i][:,:,0] for i in range(self.num_weights)]
        # end 2.0

        # 3.0 每台机器选择一个模型，数据集非IID，噪声算法是计算每台机器上的所有梯度的均值的绝对值作为高斯噪声的方差
        # self.clip_updates()
        # self.m = float(self.ClippedUpdates[0].shape[-1])
        # MeanClippedUpdates = [np.mean(self.ClippedUpdates[i], -1) for i in range(self.num_weights)]
        #
        # MeanUpdates = [abs(np.mean(MeanClippedUpdates[i])) for i in range(self.num_weights)]
        #
        # GaussianNoise = [(1.0 / self.m * np.random.normal(loc=0.0, scale=float(self.Sigma * MeanUpdates[i]), size=MeanClippedUpdates[i].shape)) for i in range(self.num_weights)]
        #
        # Sanitized_Updates = [MeanClippedUpdates[i] + GaussianNoise[i] for i in range(self.num_weights)]
        #
        # New_weights = [self.global_model[i] + Sanitized_Updates[i] for i in range(self.num_weights)]
        ...
        # 4.0 每台机器选择一个模型，数据集非IID，在一台机器上单独LDP
        self.C=FLAGS.C
        ...
        if FLAGS.method==0:
            ...
            MeanClippedUpdates = [np.mean(self.Updates[i], -1) for i in range(self.num_weights)]
        #L=1/L=1/(N/b)=b/N
        GaussianNoise = [(1/self.N * np.random.normal(loc=0.0, scale=float(self.Sigma * self.median[i]),size=MeanClippedUpdates[i].shape)) for i in range(self.num_weights)]
        ...
        # 联邦平均/联邦加法？
        #mean_Updates = [np.sum(self.Updates[i],axis=-1) for i in range(self.num_weights)]
        #mean_Updates_k = [mean_Updates[i]*0.01 for i in range(self.num_weights)]
        #New_weights = [self.global_model[i] + self.mean_Updates_k[i] for i in range(self.num_weights)]
        #New_weights = [self.global_model[i] + mean_Updates_k[i] for i in range(self.num_weights)]
        # end 联邦平均/联邦加法？
...
def load_from_directory_or_initialize_K_model(directory, FLAGS):
    '''
    ...
    FLAGS.loaded = False
    FLAGS.relearn = False
    clients_epoch = [[0 for col in range(FLAGS.n)] for row in range(FLAGS.MODEL_NUM)]
        ...
        if os.path.isfile(directory + '/specs.csv'):
            with open(directory + '/specs.csv', 'r') as csvfile:
                ...
                for i in range(len(Lines)-2):
                    clients_epoch[i]=Lines[i+1]
                Accuracy_accountant = Lines[-2]
                timeList = Lines[-1]

            if np.mean(clients_epoch) != 0:
                ...
                if math.isnan(Accuracy_accountant[-1]):
                    ...
                    fil.close()
                    FLAGS.loaded = True
                return model, Accuracy_accountant, clients_epoch,timeList, Acc, real_round, FLAGS
            else:
...
def save_progress_K_model(save_dir, model, Accuracy_accountant,PrivacyAgent, clients_epoch,timeList, FLAGS):
    '''
        ...
        :param clients_epoch: Matrix [model_NUM, clients_NUM]: The epochs about every clients in every models
        :return: nothing
        '''
    ...
    if FLAGS.relearn == False:
        ...
        with open(save_dir + '/specs.csv', 'w', newline='') as csvfile:
            ...
            if FLAGS.priv_agent == True:
                writer.writerow([0] + [PrivacyAgent.get_m(r) for r in range(len(Accuracy_accountant) - 1)])
            if FLAGS.priv_agent == False:
                writer.writerow([0] + [FLAGS.m] * (len(Accuracy_accountant) - 1))
            # 写入最新一轮里每台客户端在每个模型上训练了多少epoch
            for i in range (len(clients_epoch)):
                writer.writerow(clients_epoch[i])
            # 写入每轮里所有的正确率
...
class Flag:
    def __init__(self, n, b, e, record_privacy, m, sigma, eps, save_dir, log_dir, max_comm_rounds, gm, PrivAgent, MODEL_NUM,
                 THV, C, method):
        if not save_dir:
            save_dir = os.getcwd()
        if not log_dir:
